...
from analysis.rules.move import MINE, SAFE, Move, MoveList
    ...
    for cons in comp.constraints:
        ...
        if remaining == 0:
            for local_idx in get_indicies_from_bitmask(mask_local):
                ...
                move = Move(r=r, c=c, kind=SAFE, reason=["Singles: remaining == 0 are SAFE"], score=None)
                moves.add_move(move)
        ...
        # Rule: remaining == the size of the scope, then all unknowns in scope are MINES.
            ...
            for local_idx in get_indicies_from_bitmask(mask_local):
                ...
                move = Move(r=r, c=c, kind=MINE, reason=s, score=None) 
                moves.add_move(move)
                if stop_after_one: return 
